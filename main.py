...
# V118 visual-only web dashboard styling: preserve all handlers, accounting, database, and Arabic text logic.
# The shell uses Royal Crimson navigation, quiet neutral surfaces, and card-based spacing.
COLOR_CRIMSON = "#A52A2A"      # Primary Royal Crimson
COLOR_CRIMSON_DARK = "#650E13" # Deep crimson sidebar
COLOR_CRIMSON_DEEP = "#4A0E0E" # Login and active-state depth
COLOR_CRIMSON_SOFT = "#F8E9EA" # Subtle crimson surface
COLOR_WHITE = "#FFFFFF"
COLOR_BG_LIGHT = "#F3F5F7"
COLOR_SURFACE = "#FFFFFF"
COLOR_BORDER = "#E4E8EC"
COLOR_TEXT_DARK = "#1F2933"
COLOR_TEXT_MUTED = "#68737D"
...
class Database:
    ...
    def create_tables(self):
        ...
        default_settings = [
            ('shop_name', SHOP_NAME), ('phone', PHONE), ('location', LOCATION), ('currency', CURRENCY), ('reg_points', '20'),
...
class TrendCenterApp(ctk.CTk):
    def __init__(self):
        ...
        style.configure("Treeview", font=("Arial", 13, "bold"), rowheight=38, background=COLOR_SURFACE, fieldbackground=COLOR_SURFACE, foreground=COLOR_TEXT_DARK, borderwidth=0)
        style.configure("Treeview.Heading", font=("Arial", 14, "bold"), rowheight=40, background=COLOR_CRIMSON, foreground=COLOR_WHITE, relief="flat")
    ...
    def show_login(self):
        ...
        try:
            ...
            if os.path.exists(logo_p):
                logo_img = ctk.CTkImage(light_image=Image.open(logo_p), size=(80, 80))
                ctk.CTkLabel(banner, image=logo_img, text="").pack(side="right", padx=20)
        except:
            pass
        ctk.CTkLabel(banner, text=fix_arabic(SHOP_NAME, for_ui=True), font=("Arial", 26, "bold"), text_color=COLOR_WHITE).pack(side="right", padx=20, expand=True)
    ...
    def show_dashboard(self):
        ...
        # Persistent web-style right navigation rail.
        ...
        try:
            ...
            pass
        ctk.CTkLabel(brand, text=fix_arabic(s_name, for_ui=True), font=("Arial", 16, "bold"), text_color=COLOR_WHITE, wraplength=260).pack(pady=(0, 3))
        ctk.CTkLabel(brand, text=fix_arabic("مركز العمليات المحلي • V118", for_ui=True), font=("Arial", 11, "bold"), text_color="#F4C6C8").pack()
        ...
        ctk.CTkLabel(bottom_frame, text=fix_arabic(f"المستخدم: {self.current_user or ''}", for_ui=True), font=("Arial", 12, "bold"), text_color="#F4C6C8").pack(pady=(12, 3))
        if self.current_role == "employee":
            ctk.CTkButton(bottom_frame, text=fix_arabic("تغيير كلمة السر", for_ui=True), fg_color="#245B9C", hover_color="#1B4779", command=self.change_own_password, font=FONT_BOLD, height=42, corner_radius=10).pack(fill="x", pady=5, padx=14)
        ctk.CTkButton(bottom_frame, text=fix_arabic("تسجيل خروج", for_ui=True), fg_color="#D77A1F", hover_color="#B85E12", command=self.show_login, font=FONT_BOLD, height=42, corner_radius=10).pack(fill="x", pady=5, padx=14)
        if self.current_role == "admin":
            ctk.CTkButton(bottom_frame, text=fix_arabic("إغلاق البرنامج", for_ui=True), fg_color="#B52B34", hover_color="#8E2028", command=self.quit, font=FONT_BOLD, height=42, corner_radius=10).pack(fill="x", pady=(5, 14), padx=14)

        # Navigation remains the same; only its grouping and visual treatment change.
        nav_title = ctk.CTkLabel(self.sidebar, text=fix_arabic("مساحة العمل", for_ui=True), font=("Arial", 12, "bold"), text_color="#F4C6C8")
        nav_title.pack(anchor="e", padx=22, pady=(18, 5))
        nav_scroll = ctk.CTkScrollableFrame(self.sidebar, fg_color="transparent", scrollbar_button_color=COLOR_CRIMSON, scrollbar_button_hover_color="#C44A52")
        nav_scroll.pack(side="top", fill="both", expand=True, padx=7, pady=(0, 7))

        btns = [("نقطة البيع", self.ui_pos), ("قسم الصيانة", self.ui_maintenance), ("إدارة قطع الصيانة", self.ui_maintenance_parts), ("حوالات وفواتير", self.ui_transfers), ("نظام الولاء", self.ui_loyalty)]
        if self.current_role == "admin":
            btns += [("لوحة التحكم والتحليلات", self.ui_analytics), ("إدارة الديون والذمم ⚖️", self.ui_debts), ("إدارة المخزون", self.ui_inventory), ("المشتريات", self.ui_purchases), ("إدارة العملاء", self.ui_customers), ("إدارة العمليات", self.ui_operations_management), ("التقارير المتقدمة", self.ui_advanced_reports), ("سجل الرقابة", self.ui_audit_logs), ("المصاريف", self.ui_expenses), ("التقارير والأرباح", self.ui_reports), ("إعدادات النظام", self.ui_settings)]
        for index, (txt, cmd) in enumerate(btns):
            ctk.CTkButton(nav_scroll, text=fix_arabic(txt, for_ui=True), command=cmd, font=FONT_BOLD, height=44, corner_radius=12, fg_color="#7E171D", border_width=1, border_color="#B84A50", hover_color=COLOR_CRIMSON, anchor="e").pack(pady=4, padx=7, fill="x")
        ...
        ctk.CTkLabel(topbar, text=fix_arabic("Trend Center Jordan", for_ui=True), font=("Arial", 16, "bold"), text_color=COLOR_CRIMSON).pack(side="right", padx=22)
        ctk.CTkLabel(topbar, text=fix_arabic("مساحة تشغيل آمنة • بيانات SQLite المحلية", for_ui=True), font=("Arial", 12, "bold"), text_color=COLOR_TEXT_MUTED).pack(side="left", padx=22)
    ...
    def create_header(self, text):
        ...
        ctk.CTkLabel(header, text=fix_arabic("V118 • مساحة العمل", for_ui=True), font=("Arial", 11, "bold"), text_color="#F4C6C8").pack(pady=(10, 0))
        ctk.CTkLabel(header, text=fix_arabic(text, for_ui=True), font=("Arial", 22, "bold"), text_color=COLOR_WHITE).pack(expand=True, pady=(0, 8))
    ...
    def ui_maintenance_parts(self):
        ...
        ctk.CTkButton(top, text=str("إدخال بيانات القطع 🔐"), command=self.open_parts_entry_manager, font=FONT_BOLD, height=45, fg_color="#2e7d32").pack(side="left", padx=5)
    ...
    def open_parts_entry_manager(self):
        # Admin password protection - Use raw strings for native input dialog to prevent mirroring
        pwd = ctk.CTkInputDialog(text=str("أدخل كلمة سر الإدارة للوصول:"), title=str("حماية البيانات")).get_input()
        if pwd != "Mk@262711":
            self.show_msg("خطأ", "كلمة السر غير صحيحة")
            return
        ...
        def save_part():
            ...
            try:
                ...
                if hasattr(self, "_editing_mp_id") and self._editing_mp_id:
                    ...
                    btn_save.configure(text=str("حفظ القطعة الجديدة"), fg_color="#2e7d32")
                else:
        ...
        def load_for_edit():
            ...
            if r:
                ...
                btn_save.configure(text=str("حفظ التعديلات"), fg_color="#f57c00")

        ctk.CTkButton(scroll, text=str("تعديل القطعة المختارة"), command=load_for_edit, font=FONT_BOLD, fg_color="#f57c00", height=40).pack(pady=5, padx=20, fill="x")
        ctk.CTkButton(scroll, text=fix_arabic("حذف القطعة المختارة", for_ui=True), command=lambda: self.delete_record("maintenance_parts", self.mp_mini_tree, callback=lambda: (refresh_mini(), self.refresh_maintenance_parts()), id_index=0), font=FONT_BOLD, fg_color="#C62828", height=40).pack(pady=5, padx=20, fill="x")
        refresh_mini()
        self._editing_mp_id = None
        btn_save = ctk.CTkButton(scroll, text=fix_arabic("حفظ القطعة الجديدة", for_ui=True), command=save_part, font=FONT_BOLD, fg_color="#2e7d32", height=45)
    ...
    def checkout(self):
        ...
        try:
            ...
            if payment == "Credit":
                self.db.cursor.execute("INSERT INTO customer_debts (customer_phone, customer_name, total_debt, paid_amount, status, date, notes) VALUES (?,?,?,?,?,?,?)", 
                                       (phone, name, total, 0, 'غير مسدد', now.strftime("%Y-%m-%d"), f'بيع آجل - فاتورة POS'))
    ...
    def add_maintenance(self):
        ...
        try:
            ...
            if payment == "Credit":
                self.db.cursor.execute("INSERT INTO customer_debts (customer_phone, customer_name, total_debt, paid_amount, status, date, notes) VALUES (?,?,?,?,?,?,?)", 
                                       (ph, c, rev, 0, 'غير مسدد', now.strftime("%Y-%m-%d"), f'صيانة آجلة - {d}'))
    ...
    def ui_operations_management(self):
        ...
        comp_frame = ctk.CTkFrame(self.main_view, fg_color="#FFFFFF", corner_radius=15, border_width=1, border_color="#E0E0E0")
        ...
        def create_stat_card(parent, title, cur_val, prev_val, is_currency=True):
            card = ctk.CTkFrame(parent, fg_color="#FAFAFA", corner_radius=10, border_width=1, border_color="#E0E0E0")
            card.pack(side="right", fill="both", expand=True, padx=5, pady=5)
            ctk.CTkLabel(card, text=fix_arabic(title, for_ui=True), font=("Arial", 12, "bold"), text_color="#555555").pack(pady=(5,2))
            ...
            ctk.CTkLabel(card, text=fix_arabic(f"السابق: {prev_str}", for_ui=True), font=("Arial", 11), text_color="#777777").pack(pady=(1,5))
        ...
        chart_frame = ctk.CTkFrame(comp_frame, fg_color="#FAFAFA", corner_radius=10, border_width=1, border_color="#E0E0E0", height=130)
        chart_frame.pack(fill="x", padx=15, pady=(5, 12))
        chart_frame.pack_propagate(False)
        
        ctk.CTkLabel(chart_frame, text=fix_arabic("مقارنة مرئية: إيرادات المبيعات، الصيانة، والحوالات (الشهر الحالي مقابل السابق)", for_ui=True), font=("Arial", 11, "bold"), text_color="#444444").pack(pady=4)
        
        canvas_w, canvas_h = 600, 75
        chart_canvas = ctk.CTkCanvas(chart_frame, width=canvas_w, height=canvas_h, bg="#FAFAFA", highlightthickness=0)
        ...
        chart_canvas.create_text(120, 12, text=fix_arabic(f"الشهر السابق ({prev_prefix})", for_ui=True), font=("Arial", 10, "bold"), fill="#666666")
        ...
        chart_canvas.create_rectangle(70, 65 - p_s, 95, 65, fill="#9E9E9E", outline="")
        chart_canvas.create_rectangle(105, 65 - p_m, 130, 65, fill="#757575", outline="")
        chart_canvas.create_rectangle(140, 65 - p_t, 165, 65, fill="#424242", outline="")
        chart_canvas.create_text(118, 72, text=f"{prev_stats['rev']:.1f}", font=("Arial", 9), fill="#555555")
        
        # Current Month Bar group
        chart_canvas.create_text(420, 12, text=fix_arabic(f"الشهر الحالي ({cur_prefix})", for_ui=True), font=("Arial", 10, "bold"), fill=COLOR_CRIMSON)
        c_s = cur_stats["sales_rev"] * scale
        c_m = cur_stats["maint_rev"] * scale
        c_t = cur_stats["trans_comm"] * scale
        
        chart_canvas.create_rectangle(370, 65 - c_s, 395, 65, fill="#C44A52", outline="")
        chart_canvas.create_rectangle(405, 65 - c_m, 430, 65, fill="#A33B43", outline="")
        chart_canvas.create_rectangle(440, 65 - c_t, 465, 65, fill="#822C34", outline="")
        chart_canvas.create_text(418, 72, text=f"{cur_stats['rev']:.1f}", font=("Arial", 9, "bold"), fill=COLOR_CRIMSON)
        
        # Legend
        leg_f = ctk.CTkFrame(chart_frame, fg_color="transparent")
        leg_f.pack(fill="x", padx=10)
        ctk.CTkLabel(leg_f, text=fix_arabic("◼ مبيعات  ◼ صيانة  ◼ عمولات حوالات", for_ui=True), font=("Arial", 10), text_color="#555555").pack()

        # Financial Policies Panel (V112)
        p_frame = ctk.CTkFrame(self.main_view, fg_color="#F5F5F5", corner_radius=15, border_width=1, border_color="#E0E0E0")
        ...
        ctk.CTkButton(p_frame, text=fix_arabic("حفظ السياسات المالية", for_ui=True), command=self.save_financial_policies, font=FONT_BOLD, fg_color="#2E7D32", height=40, width=200).pack(pady=10)
    ...
    def refresh_operations_tree(self):
        ...
        try:
            ...
            queries = [
                ...
                ("purchases", f"SELECT COALESCE(user,'-'), 'Cash', (qty * cost), TRIM(COALESCE(name,'') || CASE WHEN COALESCE(supplier,'')<>'' THEN ' - ' || supplier ELSE '' END), 'مشتريات', date, id FROM purchases {where} ORDER BY date DESC, id DESC LIMIT 200", params),
    ...
    def ui_settings(self):
        ...
        f_shop = ctk.CTkFrame(scroll, corner_radius=15, border_width=1, fg_color="#EEEEEE"); f_shop.pack(fill="x", pady=10, padx=10)
        ...
        self.s_entries = {}
        for k, label in [('shop_name', "اسم المحل"), ('phone', "رقم الهاتف"), ('location', "الموقع")]:
        ...
        ctk.CTkButton(f_shop, text=fix_arabic("حفظ إعدادات المحل", for_ui=True), command=self.save_settings, font=FONT_BOLD, fg_color="#2e7d32", height=45, width=250).pack(pady=20)

        # Configurable Paths Management Panel (Database & Invoices folders)
        f_paths = ctk.CTkFrame(scroll, corner_radius=15, border_width=1, fg_color="#FDF2F2"); f_paths.pack(fill="x", pady=10, padx=10)
        ...
        for k, label, val in [('db_path', "مسار ملف قاعدة البيانات (SQLite):", current_db), ('invoice_dir', "مجلد حفظ صور الفواتير:", current_inv)]:
            ...
            ctk.CTkButton(r, text=fix_arabic("استعراض...", for_ui=True), command=make_browse_cmd(), font=FONT_BOLD, width=100, height=42, fg_color="#1565C0").pack(side="left", padx=5)
        ...
        ctk.CTkButton(btn_row, text=fix_arabic("تعديل المستخدم المختار", for_ui=True), command=self.edit_user_ui, font=FONT_BOLD, fg_color="#f57c00", height=45).pack(side="right", padx=5)
        ctk.CTkButton(btn_row, text=fix_arabic("حذف المستخدم المختار", for_ui=True), command=self.delete_user, font=FONT_BOLD, fg_color="#C62828", height=45).pack(side="right", padx=5)
    ...
    def ui_expenses(self):
        ...
        f = ctk.CTkFrame(self.main_view, fg_color="transparent"); f.pack(fill="x", padx=15, pady=5)
        self.e_desc = ctk.CTkEntry(f, placeholder_text=fix_arabic("الوصف", for_ui=True), width=300, height=45, justify="right", corner_radius=10, font=FONT_NORMAL_BOLD); self.e_desc.pack(side="right", padx=5)
        self.e_amt = ctk.CTkEntry(f, placeholder_text=fix_arabic("المبلغ", for_ui=True), height=45, justify="right", corner_radius=10, font=FONT_NORMAL_BOLD); self.e_amt.pack(side="right", padx=5)
        ...
        self.exp_tree = ttk.Treeview(self.main_view, columns=("id", "date", "amount", "desc"), show="headings")
        for col, head in zip(self.exp_tree["columns"], ["ID", "التاريخ", "المبلغ", "الوصف"]): 
        ...
        def refresh_exp():
            ...
            self.db.cursor.execute("SELECT id, date, amount, desc FROM expenses ORDER BY id DESC")
            for r in self.db.cursor.fetchall(): self.exp_tree.insert("", "end", values=(r[0], r[1], f"{r[2]:.2f}", fix_arabic(r[3], for_ui=True)))
        
        refresh_exp()
        
        btn_f = ctk.CTkFrame(self.main_view, fg_color="transparent"); btn_f.pack(fill="x", padx=15, pady=5)
        ctk.CTkButton(btn_f, text=fix_arabic("تعديل المصروف المختار", for_ui=True), command=self.edit_selected_expense, font=FONT_BOLD, fg_color="#f57c00", height=40).pack(side="right", padx=5)
        ctk.CTkButton(btn_f, text=fix_arabic("حذف المصروف المختار", for_ui=True), command=lambda: self.delete_record("expenses", self.exp_tree, callback=refresh_exp, id_index=0), font=FONT_BOLD, fg_color="#d32f2f", height=40).pack(side="right", padx=5)
    ...
    def edit_selected_expense(self):
        ...
        eid, old_amt, old_desc = vals[0], vals[2], vals[3]
        
        win = ctk.CTkToplevel(self); win.title(str("تعديل مصروف")); win.geometry("400x300"); win.attributes("-topmost", True); win.grab_set()
        ctk.CTkLabel(win, text=str("تعديل بيانات المصروف"), font=FONT_BOLD, text_color=COLOR_CRIMSON).pack(pady=15)
        e_desc = ctk.CTkEntry(win, width=300, height=45, justify="right"); e_desc.pack(pady=5); e_desc.insert(0, old_desc)
        e_amt = ctk.CTkEntry(win, width=300, height=45, justify="right"); e_amt.pack(pady=5); e_amt.insert(0, str(old_amt).split()[0])
        
        def save():
            try:
                d, a = e_desc.get().strip(), self.positive_number(e_amt.get(), "المبلغ")
                self.db.cursor.execute("UPDATE expenses SET desc=?, amount=? WHERE id=?", (d, a, eid))
                self.db.conn.commit(); self.log_action("تعديل مصروف", "expenses", f"ID: {eid}"); win.destroy(); self.ui_expenses()
            except Exception as exc: self.show_msg("خطأ", str(exc))
        ctk.CTkButton(win, text=str("حفظ التعديلات"), command=save, font=FONT_BOLD, fg_color="#2e7d32", height=45).pack(pady=20)
    ...
    def ui_analytics(self):
        ...
        f_daily = ctk.CTkFrame(scroll, corner_radius=15, fg_color="#E3F2FD", border_color="#1565C0", border_width=2); f_daily.pack(fill="x", pady=10, padx=10)
        ...
        f_cust = ctk.CTkFrame(scroll, corner_radius=15, border_width=1, fg_color="#FFF9C4", border_color="#FBC02D"); f_cust.pack(fill="x", pady=10, padx=10)
        ctk.CTkLabel(f_cust, text=fix_arabic("أكثر 3 عملاء تعاملاً معنا (Top 3 Customers)", for_ui=True), font=FONT_BOLD, text_color="#F57F17").pack(pady=10)
    ...
    def ui_debts(self):
        ...
        ctk.CTkButton(btn_top, text=str("إضافة تفاصيل دين جديدة ➕"), command=self.open_add_new_debt_details_ui, font=FONT_BOLD, fg_color="#2e7d32", height=52, width=280).pack(side="right", padx=10)
        ...
        ctk.CTkButton(btn_c, text=str("الاطلاع على التفاصيل 📋"), command=lambda: self.show_debt_details_v111("customer"), font=FONT_BOLD, fg_color="#1565C0", height=45, width=180).pack(side="right", padx=10)
        ctk.CTkButton(btn_c, text=str("تسديد دفعة عميل"), command=lambda: self.open_pay_debt_ui("customer"), font=FONT_BOLD, fg_color="#f57c00", height=45, width=180).pack(side="right", padx=10)
        ctk.CTkButton(btn_c, text=str("حذف السجل"), command=lambda: self.delete_record("customer_debts", self.cd_tree, callback=self.refresh_debts), font=FONT_BOLD, fg_color="#c62828", height=45, width=150).pack(side="left", padx=10)
        ...
        ctk.CTkButton(btn_s, text=str("الاطلاع على التفاصيل 📋"), command=lambda: self.show_debt_details_v111("supplier"), font=FONT_BOLD, fg_color="#1565C0", height=45, width=180).pack(side="right", padx=10)
        ctk.CTkButton(btn_s, text=str("تسديد دفعة مورد"), command=lambda: self.open_pay_debt_ui("supplier"), font=FONT_BOLD, fg_color="#f57c00", height=45, width=180).pack(side="right", padx=10)
        ctk.CTkButton(btn_s, text=str("حذف السجل"), command=lambda: self.delete_record("supplier_debts", self.sd_tree, callback=self.refresh_debts), font=FONT_BOLD, fg_color="#c62828", height=45, width=150).pack(side="left", padx=10)
    ...
    def show_debt_details_v111(self, dtype="customer"):
        ...
        f = ctk.CTkFrame(win, fg_color="#EEEEEE", corner_radius=10); f.pack(fill="both", expand=True, padx=20, pady=10)
    ...
    def open_add_new_debt_details_ui(self):
        ...
        ctk.CTkLabel(win, text=str("إدخال بيانات الدين الجديد"), font=("Arial", 20, "bold"), text_color=COLOR_CRIMSON).pack(pady=20)
        ...
        ctk.CTkButton(win, text=str("حفظ بيانات الدين"), command=save, font=FONT_BOLD, fg_color="#2e7d32", height=50).pack(pady=25, padx=50, fill="x")
    ...
    def open_pay_debt_ui(self, dtype="customer"):
        ...
        def pay():
            try:
                ...
                self.db.cursor.execute("INSERT INTO debt_payments (debt_id, debt_type, amount, date, time, notes) VALUES (?,?,?,?,?,?)", (rid, dtype, amt, d_str, t_str, note_e.get().strip()))
        ...
        ctk.CTkButton(win, text=str("تأكيد الدفع"), command=pay, font=FONT_BOLD, fg_color="#2e7d32", height=45).pack(pady=20)
    ...
    def ui_reports(self):
        ...
        ctk.CTkLabel(f_top, text=fix_arabic("من:", for_ui=True), font=FONT_BOLD, text_color=COLOR_CRIMSON).pack(side="right", padx=2)
        ctk.CTkButton(f_top, text=fix_arabic("جرد الكاش اليومي", for_ui=True), command=self.show_cash_reconciliation, font=FONT_BOLD, fg_color="#2e7d32", height=40).pack(side="left", padx=5)
        ctk.CTkButton(f_top, text=fix_arabic("تقرير الأرباح والخسائر P&L", for_ui=True), command=self.show_p_and_l_statement, font=FONT_BOLD, fg_color="#006064", height=40).pack(side="left", padx=5)
    ...
    def refresh_reports(self):
        ...
        try:
            ...
            def scalar(query, values=params):
                self.db.cursor.execute(query + (" " + where if where else ""), values); return float(self.db.cursor.fetchone()[0] or 0.0)
            ...
            exp = scalar("SELECT SUM(amount) FROM expenses")
            purch = scalar("SELECT SUM(qty * cost) FROM purchases")
            ...
            # 1. Main Summary Table (Gray/White alternating)
            f_sum = ctk.CTkFrame(self.rep_body, corner_radius=15, border_width=1, fg_color="#EEEEEE"); f_sum.pack(fill="x", pady=10, padx=10)
            items = [
                ("إجمالي المبيعات", s_rev, "#1565C0"),
                ("قيمة المنتجات المباعة (من رأس المال - COGS)", s_cogs, "#C62828"),
                ("ربح المبيعات الصافي", s_prof, "#2E7D32"),
                ("إجمالي إيرادات الصيانة", m_rev, "#1565C0"),
                ("تكلفة قطع الصيانة", m_cost, "#C62828"),
                ("ربح الصيانة الصافي", m_prof, "#2E7D32"),
                ("إجمالي عمولات الحوالات والفواتير", t_comm, "#00838F"),
                ("إجمالي المصاريف", exp, "#C62828"),
                ("إجمالي المشتريات (الكمية × تكلفة القطعة)", purch, "#5D4037")
            ]
            for i, (label, val, color) in enumerate(items):
                ...
                ctk.CTkLabel(row, text=fix_arabic(f"{label}", for_ui=True), font=FONT_BOLD, text_color=COLOR_TEXT_DARK).pack(side="right", padx=20, pady=8)
                ctk.CTkLabel(row, text=fix_arabic(f"{val:.2f} {CURRENCY}", for_ui=True), font=FONT_BOLD, text_color=color).pack(side="left", padx=20, pady=8)
            
            # 2. Inventory Valuation
            f_inv = ctk.CTkFrame(self.rep_body, corner_radius=15, border_width=2, border_color="#2e7d32", fg_color="#F1F8E9"); f_inv.pack(fill="x", pady=15, padx=10)
            ctk.CTkLabel(f_inv, text=fix_arabic("تقييم المخزون الحالي", for_ui=True), font=FONT_BOLD, text_color="#1B5E20").pack(pady=5)
            row_inv = ctk.CTkFrame(f_inv, fg_color="transparent"); row_inv.pack(fill="x", padx=20, pady=10)
            ctk.CTkLabel(row_inv, text=fix_arabic(f"قيمة المخزون (سعر الشراء): {stock_buy:.2f} {CURRENCY}", for_ui=True), font=FONT_NORMAL_BOLD, text_color="#1B5E20").pack(side="right", padx=20)
            ctk.CTkLabel(row_inv, text=fix_arabic(f"القيمة المتوقعة (سعر البيع): {stock_sell:.2f} {CURRENCY}", for_ui=True), font=FONT_NORMAL_BOLD, text_color="#1565C0").pack(side="left", padx=20)
            
            # 3. Net Profit (Large Gold Box) - Expanded padding for better visibility
            f_profit = ctk.CTkFrame(self.rep_body, corner_radius=15, border_width=2, border_color="#FBC02D", fg_color="#FFFDE7"); f_profit.pack(fill="x", pady=25, padx=10)
            ctk.CTkLabel(f_profit, text=fix_arabic("صافي الربح الفعلي", for_ui=True), font=("Arial", 22, "bold"), text_color="#F57F17").pack(pady=(15, 10))
            ctk.CTkLabel(f_profit, text=fix_arabic(f"{net_profit:.2f} {CURRENCY}", for_ui=True), font=("Arial", 42, "bold"), text_color="#2E7D32").pack(pady=(0, 20))
    ...
    def show_cash_reconciliation(self):
        ...
        self.db.cursor.execute("SELECT value FROM settings WHERE key='opening_balance'")
        open_bal = float(self.db.cursor.fetchone()[0] or 0.0)
        
        # الكاش المفترض في الدرج:
        # - رصيد البداية + المبيعات النقدية + إيرادات الصيانة النقدية
        # - دخول حوالة نقداً + دفع فاتورة نقداً
        # - خروج حوالة (تخصم من الدرج بغض النظر عن طريقة الدفع)
        expected_cash = open_bal + s_cash + m_cash + t_in_cash + b_pay_cash - t_out_net_all
        total_liquidity = expected_cash + total_visa + total_cliq
        ...
        f_open = ctk.CTkFrame(scroll, fg_color="#FFF9C4", corner_radius=15, border_width=1, border_color="#FBC02D")
        f_open.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(f_open, text=fix_arabic("رصيد بداية الصندوق اليومي", for_ui=True), font=FONT_BOLD, text_color="#F57F17").pack(pady=5)
        ...
        def save_opening():
            try:
                ...
                self.db.cursor.execute("UPDATE settings SET value=? WHERE key='opening_balance'", (str(val),))
                self.db.conn.commit()
                win.destroy(); self.show_cash_reconciliation()
            except: pass
            
        ctk.CTkButton(row_o, text=fix_arabic("تحديث الجرد", for_ui=True), command=save_opening, font=FONT_BOLD, width=140, height=40, fg_color="#F57F17", hover_color="#E65100").pack(side="left", padx=10)

        # Breakdown card
        f_break = ctk.CTkFrame(scroll, fg_color=COLOR_BG_LIGHT, corner_radius=15)
        f_break.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(f_break, text=fix_arabic("إجمالي السيولة حسب طريقة الدفع", for_ui=True), font=FONT_BOLD, text_color="#1565C0").pack(pady=8)
        ...
        for lbl, val in pm_items:
            ...
            ctk.CTkLabel(row, text=f"{val:.2f} {CURRENCY}", font=FONT_BOLD, text_color="#2E7D32" if val >= 0 else COLOR_CRIMSON).pack(side="left", padx=10)
        ...
        f_total = ctk.CTkFrame(scroll, fg_color="#E8F5E9", corner_radius=15, border_width=2, border_color="#2E7D32")
        f_total.pack(fill="x", padx=10, pady=10)
        
        row_c = ctk.CTkFrame(f_total, fg_color="transparent"); row_c.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(row_c, text=fix_arabic("الكاش المفترض في الدرج:", for_ui=True), font=("Arial", 16, "bold"), text_color="#1B5E20").pack(side="right")
        ctk.CTkLabel(row_c, text=f"{expected_cash:.2f} {CURRENCY}", font=("Arial", 20, "bold"), text_color="#2E7D32").pack(side="left")
        
        row_l = ctk.CTkFrame(f_total, fg_color="transparent"); row_l.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(row_l, text=fix_arabic("إجمالي السيولة (كاش + بنك):", for_ui=True), font=("Arial", 18, "bold"), text_color="#0D47A1").pack(side="right")
        ctk.CTkLabel(row_l, text=f"{total_liquidity:.2f} {CURRENCY}", font=("Arial", 24, "bold"), text_color="#1565C0").pack(side="left")
